var b, bp, wp, bp_tryout, wp_tryout, user_color, user_color_tryout, m
var tiles = [];
...
function redraw_moves() {
    for (var i = 0; i < bp_tryout.length; i++) {
        if (bp_tryout[i] == 1) {
            add_piece_tryout(i, 0)
            $("#tryout_tile_" + i.toString()).append($("<div>", { "class": "blackShadow" }))
            $(".blackShadow").remove();
        } // black draw
        if (wp_tryout[i] == 1) {
            add_piece_tryout(i, 1)
...
function user_move(game_info, tryoutgame_info) {
    ...
    $('.tile').off('click').on('click', function (e) {
        ...
        if (winning_pieces.length == N) {
            ...
            end_game(game_info, tryoutgame_info, 'win')
            $("#tryout").hide()
        }
        else if (check_draw()) {
            log_data({ "event_type": "draw", "event_info": { "bp": bp.join(""), "wp": wp.join("") } })
            $('.headertext h1').text('Game over, draw').css('color', '#000000');
            end_game(game_info, tryoutgame_info, 'draw')
            $("#tryout").hide()
        }
        else {
            make_opponent_move(game_info, tryoutgame_info)
        }
    });

    // Dongjae tryoutboard
    $("#tryout").show().css({ "display": "inline" }).off("click").on("click", function () {
        if (toggle_tryout_button) {
            $(".tryout-canvas").empty()
            create_tryout_board()
            bp_tryout = bp.slice()
            wp_tryout = wp.slice()
            user_color_tryout = user_color

            redraw_moves()
            // tile_ind = parseInt(e.target.id.replace("tryout_tile_", ""));
            // add_piece_tryout(tile_ind, user_color_tryout);
            // show_last_move_tryout(tile_ind, user_color_tryout);
            ...
            $('.popuptext h2').text('Your turn. You play ' + (user_color_tryout == 0 ? 'black' : 'white') + ".");
            start_tryout_game(tryoutgame_info, game_info)
        } else {
            $("#tryout").text('You cannot open anymore')
            $("#tryout").css('background-color', '#EEEEEEE')
            $("#tryout").css('color', 'red')
            $("#tryout").show().css({ "display": "inline" }).off("click")
        }
    });
}
...
function user_tryout_move(game_info) {
    ...
    $('#close-button').off("click").on('click', function (e) {
        log_data_tryout({ "event_type": "close tryout", "event_info": { "game_num": game_info.num, "is_practice": game_info.practice, "level": level } })
        $("#myModal").css("display", "none")
        toggle_tryout_button = !toggle_tryout_button
    });
    ...
    winning_pieces_already = check_tryout_win(user_color_tryout)
    winning_pieces_mainboard = check_win(user_color_tryout)
    draw_pieces_mainboard = check_draw(user_color_tryout)
    if (!(winning_pieces_already.length == N) || (winning_pieces_mainboard || draw_pieces_mainboard)) {
        $('.tileTryout').off('click').on('click', function (e) {
            ...
            tile_ind = parseInt(e.target.id.replace("tryout_tile_", ""));
            log_data_tryout({ "event_type": "user move", "event_info": { "tile": tile_ind, "user_color": (user_color_tryout == 0 ? 'black' : 'white'), "bp": bp_tryout.join(""), "wp": wp_tryout.join("") } })
            add_piece_tryout(tile_ind, user_color_tryout);
            show_last_move_tryout(tile_ind, user_color_tryout);
            $(".tryout_clickprompt").hide();
            dismissed_tryout_click_prompt = true;
            winning_pieces = check_tryout_win(user_color_tryout)
            if (winning_pieces.length == N) {
                show_win_tryout(user_color_tryout, winning_pieces)
                ...
                user_color_tryout = (user_color_tryout + 1) % 2
...
function make_user_opponent_tryout_move(game_info) {
    ...
    $('.popuptext h2').text('Opponent\'s turn. Now you play ' + (user_color_tryout == 0 ? 'black' : 'white') + ".");
    ...
    if (!(winning_pieces_already.length == N) || (winning_pieces_mainboard || draw_pieces_mainboard)) {
        $('.tileTryout').off('click').on('click', function (e) {
            ...
            $('.tryout-canvas, .tryout-canvas div').css('cursor', 'default');
            tile_ind = parseInt(e.target.id.replace("tryout_tile_", ""));
            log_data_tryout({ "event_type": "opponent move", "event_info": { "tile": tile_ind, "user_color": (user_color_tryout == 0 ? 'black' : 'white'), "bp": bp_tryout.join(""), "wp": wp_tryout.join("") } })
            add_piece_tryout(tile_ind, user_color_tryout);
            show_last_move_tryout(tile_ind, user_color_tryout);
            $(".tryout_clickprompt").hide();
            dismissed_tryout_click_prompt = true;
...
function make_opponent_move(game_info, tryoutgame_info) {
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            ...
            if (winning_pieces.length == N) {
                ...
                end_game(game_info, tryoutgame_info, 'opponent win')
                $("#tryout").hide()
            }
            else if (check_draw()) {
                log_data({ "event_type": "draw", "event_info": { "bp": bp.join(""), "wp": wp.join("") } })
                ...
                user_move(game_info, tryoutgame_info)
            }
        }, 1000);
    }, 0)
...
function start_tryout_game(tryoutgame_info, game_info) {
    ...
    if (!(winning_pieces_already.length == N) || (winning_pieces_mainboard || draw_pieces_mainboard)) {
        ...
        // $('#instructions').hide();
        // $('.overlayed').hide();
        $('.tryout_gamecount').text(`${tryoutgame_info.practice ? "Practice" : "Game"} ${tryoutgame_info.num + 1} of ${tryoutgame_info.amount}`);
...
function start_game(game_info, tryoutgame_info) {
    if (!game_info.num) {
        game_info.num = 0;
    ...
    create_board()
    create_tryout_board()
    $("#tryout").text('Open the tryout board')
    $("#tryout").css('background-color', '#999999')
    $("#tryout").css('color', '#ffffff')
    if (user_color == 0)
        user_move(game_info, tryoutgame_info)
    else
        make_opponent_move(game_info, tryoutgame_info)
...
function end_game(game_info, tryoutgame_info, result) {
    ...
    $("#nextgamebutton").show().css({ "display": "inline" }).off("click").on("click", function () {
        ...
        game_info.num++;
        if (game_info.num < game_info.amount)
            start_game(game_info, tryoutgame_info)
        else {
            $('.headertext h1').text('');
            current_instruction_nr++;
            perform_instruction();
...
function perform_instruction() {
    ...
    if (instructions[current_instruction_nr].game_info) {
        start_game(instructions[current_instruction_nr].game_info, instructions[current_instruction_nr].tryoutgame_info);
        return;
    }
    $('.overlayed').show();
    $('#instructions').show();
